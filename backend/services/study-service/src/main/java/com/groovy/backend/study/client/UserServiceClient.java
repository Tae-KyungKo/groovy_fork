...
/**
 * MSA 전환(study-service 추출): Study를 groovy(레거시)에서 분리하면서 처음으로 생긴 서비스 간
 * 동기 HTTP 호출이다. leaderName/applicantName 같은 표시용 이름은 이 서비스에 User 테이블이
 * 없으므로 identity-service에 물어봐야 한다(identity-service UserController#getNames 참고).
 *
 * identity-service가 죽어 있으면 이름 없이("null") 나머지 데이터라도 보여주는 편이, 스터디
