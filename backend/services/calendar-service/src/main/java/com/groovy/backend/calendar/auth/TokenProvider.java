package com.groovy.backend.calendar.auth;
...
/**
 * MSA 전환(calendar-service 추출): study-service/notification-service의 TokenProvider와
 * 동일한 패턴. JwksKeyLocator가 identity-service의 공개키로 서명을 검증만 한다. principal을
 * email이 아니라 userId(Long)로 바로 채운다 — 이 서비스에는 User 테이블이 없어서 email로는
