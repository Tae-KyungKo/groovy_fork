...
public class CalendarService {
	...
	public CalendarEventResponse addSchedule(String email, CalendarCreateRequest request) {
		...
		return CalendarEventResponse.fromPersonal(calendarRepository.save(calendar));
	}
	...
	public List<CalendarEventResponse> getIntegratedCalendar(String email) {
		...
		List<CalendarEventResponse> personalSchedules = calendarRepository.findByUserId(user.getId()).stream()
			.map(CalendarEventResponse::fromPersonal)
			.toList();

		List<CalendarEventResponse> studySchedules = applicationRepository
			...
			.map(CalendarEventResponse::fromStudy)
			.toList();
